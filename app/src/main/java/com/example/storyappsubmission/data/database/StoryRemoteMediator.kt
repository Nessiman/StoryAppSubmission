...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            val responseData = apiService.getStories("Bearer $userToken", page, state.config.pageSize).listStory
            val endOfPaginationReach = responseData.isNullOrEmpty()
            ...
            database.withTransaction {
                ...
                val fillteredResponseData =  responseData!!.filterNotNull()
                val keys = fillteredResponseData.map {
                    RemoteKeys(id = it.id!!, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                database.storyDao().insertStory(fillteredResponseData)
            }
            MediatorResult.Success(endOfPaginationReached = responseData!!.isEmpty())
        }catch (e: Exception){
